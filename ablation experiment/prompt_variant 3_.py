# When testing variants 3, 6 and 7, include them in the prompt.py file.
...
# Evidence-bounded generation ablation: omit support, abstention, and audit-based constraints.
SYSTEM_PROMPTS["qa-cot"] = """You are an expert in retrieval QA and Chain of Thought reasoning.
Please provide clear reasoning steps before giving the final answer.
[General Reasoning Rules]
1. Chain question handling: For "A of B of C" structures, you must break them down step by step:
   - Step 1: First find the B entity corresponding to C
   - Step 2: Then find the A entity corresponding to the B entity
2. Clear relationship direction:
   - "X of Y" = Y possesses X
   - "opposition of X" = groups that oppose X
   - "child of X" = X's child
   - "father of X" = X's father
3. Homonymous entity distinction: If there are multiple entities with the same name in the context, you must distinguish them clearly based on the identity, time, and relationship characteristics described in the context
...
class PromptGenerator:
    ...
    def _wrap_llama3_template(self, user_content):
        """
        Wrap user content with the selected system prompt using the LLaMA 3.1
        chat template.
        """
...
"""
    ...
    def _generate_prompt(self, user_prompt):
        # Apply the shared chat-template wrapper.
...
3. **STRICT FORMAT RULE**: Every triple must be written as (subject, predicate, object)
   ...
   - Do NOT use square brackets [] or curly brackets {{}}
...
Task Description:
Given knowledge triples,a question and a context, your task is to select the most accurate and relevant answer from the provided options.
Follow the steps:
1. Analyze the **Question** carefully.
...
"""
    ...
    # Evidence-bounded generation ablation: retain the shared interface but omit generation constraints.
    # Preserve the shared signature while omitting audit metadata and context-bounded generation rules.
    def generate_qa_prompt_normal_cot(
            self,
            context,
            question,
            options=None,
            facts=None,
            option_counts_str=None,
            force_idk=False,
        ...
        normal_w_facts_prompt = """
Task Description:
Answer the question based on the given context and your reasoning ability.
Please return in JSON format: {{ "Reason": "State your reasoning process.", "Answer": "Your final answer" }}
Question:
{question}
Context:
{context}
...
Question: {question}
Context: {context}
CoT-Answer: {{ "Reason": "...", "Answer": "..." }}
"""
        return self._generate_prompt(normal_wo_facts_prompt_backup.format(question=question, context=context))

    # Evidence-bounded generation ablation: retain the shared interface but omit generation constraints.
    def generate_qa_prompt_schedule_cot(
            self,
            context,
            question,
            facts,
            ...
            force_idk=False,
            sufficiency_score=None,
            conflict_score=None,               # Accepted for interface compatibility; not used here.
            sufficiency_threshold=0.0,         # Accepted for interface compatibility; not used here.
            conflict_threshold=1.0,            # Accepted for interface compatibility; not used here.
            **kwargs
    ):
        normal_w_facts_prompt = """
Task Description:
Answer the question based on the given context.
For chain questions: Break down into sub-questions and verify each step.
For relationship questions: Strictly follow the direction rules in the system prompt.
Please return in JSON format: {{ "Reason": "List your reasoning steps.", "Answer": "Your final answer" }}
Question:
{question}
Context:
{context}
