...
def repair_and_format_json(text, context=None, options=None):
    ...
    # Fallback strategy for malformed answers
    ...
    if options and context and not is_valid_answer(answer_val, options):
        ...
        if sorted_options:
            ...
            return json.dumps(
                {"Reason": fallback_reason, "Answer": fallback_answer},
                ensure_ascii=False
            )

    return json.dumps(
        {"Reason": reason_val, "Answer": answer_val},
        ensure_ascii=False
    )
...
async def process_single_batch_with_retry(rag, batch_ds):
    ...
    # Step 1: Build query-anchored relational evidence states
    ...
    # Step 2: Filter top-K evidence chunks admissible from context
    ...
    # Associate relational evidence states with corresponding context chunks
    ...
    raw_preds_dict = await rag.get_predictions(
        batch_ds,
        facts_dict,
        **sampling_kwargs
    )
...
async def run_in_batches(dataset, batch_size=5):
    ...
    for i in range(start_batch_idx, num_batches):
        ...
        try:
            ...
            facts, chunks, raw_preds_dict = await process_single_batch_with_retry(
                rag,
                batch_ds
            )
            ...
            for idx in range(len(batch_ds)):
                ...
                pred = (
                    printable_preds[idx]
                    if idx < len(printable_preds)
                    else "No valid structured output obtained"
                )
            ...
            total_correct += (
                len(batch_ds) *
                (batch_results['exact_match'] / 100)
            )

            if total_samples_run > 0:
                current_overall_em = (
                    total_correct / total_samples_run
                ) * 100
