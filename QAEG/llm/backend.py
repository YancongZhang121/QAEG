...
class LLMBackend:
    ...
    def __init__(self, backend_type: str, model_name: str, **backend_config):
        ...
        Args:
            backend_type: Type of LLM backend (openai, hf, llamafactory, ollama, custom_server)
        ...
        # 【新增】懒加载自定义服务器后端
        if LLMBackend.BACKENDS["custom_server"] is None:
            try:
                from .custom_server import custom_server_complete
                LLMBackend.BACKENDS["custom_server"] = custom_server_complete
            except ImportError:
                logger.warning("Custom server module not available")
